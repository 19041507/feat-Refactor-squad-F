...
for (const page of pages) {
  test(`${page}: entrada, imagens, estilos e links locais existem`, async () => {
    let html;
    try { html = await readFile(resolve(root, page), 'utf8'); }
    catch { assert.fail(`Página não encontrada na estrutura final: ${page}`); }
    for (const [, ref] of html.matchAll(/(?:href|src)="([^"]+)"/g)) {
      if (ref.startsWith('#') || /^[a-z]+:/i.test(ref)) continue;
      const file = ref.startsWith('/assets/') ? resolve(`.${ref}`)
        : ref.startsWith('/') ? resolve(root, `.${ref}`)
